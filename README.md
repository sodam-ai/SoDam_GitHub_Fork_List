...
> 기준일: 2026-09-07(최초 작성) → **2026-09-11 갱신**: `langflow Public.md` 대조 후 신규 포크 20개 추가 반영, 총 191개 · 대상 계정: 기존 `sodam-ai` + 신규 자료의 `SoDam-Fork` · 전부 공개(Public)
> 이 문서는 GitHub에 직접 접속해서 저장소 정보를 하나하나 확인한 뒤 작성했으며, 새로 포크가 추가될 때마다 다시 조회해서 갱신하고 있습니다.

---

## 이 문서를 읽기 전에 — 꼭 알아두면 좋은 말 6가지

컴퓨터나 개발을 한 번도 안 해보신 분도 이해할 수 있도록, 아래에서 자주 나오는 낱말을 먼저 풀어드립니다. (한 번만 읽고 넘어가시면 아래 191개 설명이 훨씬 쉬워집니다.)
...
> ⚠️ 아래 목록에 있는 저장소는 전부 **포크**입니다. 즉 사용자님이 직접 만든 프로그램이 아니라, 남이 만든 프로그램을 계정에 복사해 둔 것입니다. 각 항목마다 "원본" 링크도 같이 달아뒀으니, 누가 처음 만들었는지도 함께 확인하실 수 있습니다.

---

## 전수 점검 결과 (누락·오류·모순 확인)

이번에 다시 하나하나 GitHub에 직접 물어봐서(자동 조회) 확인한 결과입니다.

- ✅ **총 개수 확인**: 191개(기존 171개 + 이번 신규 20개), 업로드된 두 목록 기준 전부 공개(Public) 저장소이며 비공개는 없습니다.
- 🆕 **신규 추가 3건 반영 (2026-09-07, 대화 중 두 차례에 걸쳐 추가 확인됨)**: `agents-UI-Design`(원본 `wshobson/agents` — 포크하면서 이름을 바꾸신 것으로 보입니다), `ui`(원본 `shadcn-ui/ui` — 웹 화면 부품 모음으로 유명한 "shadcn/ui"), `ui-ux-pro-max-skill`(원본 `nextlevelbuilder/ui-ux-pro-max-skill`). 삭제되거나 사라진 포크는 없었습니다.
- 🆕 **신규 추가 20건 반영 (2026-09-11, `langflow Public.md`와 기존 문서 대조)**: `bookforge`, `claude-code-tips`, `claude-seo`, `diagram-design`, `ECC`, `ghostty`, `herdr`, `im-not-ai`, `k-skill`, `open-command`, `open-seo`, `openclaude`, `orca`, `public-apis-4Kr`, `reelforge`, `solo-skills`, `stitch-skills`, `store-screenshots`, `threeui`, `watermarks-remover`. 기존 문서와 중복되는 25개는 다시 추가하지 않았습니다.
- ✅ **모순 없음**: 포크 목록과 원본 정보 사이에 서로 어긋나는 내용은 없었습니다.
- 🔧 **정정 1건**: 지난 답변에서 `cc-telegram-bridge`는 "원본 정보를 확인할 수 없음"으로 안내드렸는데, 이번에 다시 확인하니 원본은 `shacede/cc-telegram-bridge`였습니다. (그 저장소는 다시 `ebibibi/ebi-agent-chat-relay`라는 저장소에서 포크된 것으로 보입니다.) 아래 목록에는 정정된 정보로 반영했습니다.
- 📝 **설명 보완 9건**: 아래 9개 저장소는 GitHub에 요약 설명이 아예 입력되어 있지 않아서, 저장소 안의 실제 문서(README 등)를 직접 열어 확인한 뒤 설명을 작성했습니다 — `autoTHREADS`, `openworker`, `fablelayer`, `auto_kairos`, `fontagent`, `cokacdir`, `harness-100`, `autoresearch-skill`, `memory-bank`.
- ⚠️ **주의가 필요한 저장소 1건**: `openkakao-cli`는 원본 제작자가 "더 이상 관리하지 않으며(DEPRECATED), 최신 카카오톡에서는 로그인이 안 된다"고 직접 밝혀 둔 상태입니다. 지금 그대로 쓰기는 어렵습니다.
- ✅ **원본 연결 정보 확인**: 기존 171개에 더해 이번 신규 20개도 업로드된 `langflow Public.md`에 `Forked from` 원본 저장소가 명시되어 있어, 원본 링크를 함께 반영했습니다.
...
| # | 분류 | 개수 |
|---|---|---|
| 1 | [AI 코딩 비서 본체](#1-ai-코딩-비서-본체-9개) | 9 |
| 2 | [AI 코딩 비서를 더 강력하게 만들어주는 부속 도구](#2-ai-코딩-비서를-더-강력하게-만들어주는-부속-도구-63개) | 63 |
| 3 | [독자적인 AI 에이전트 프레임워크 / 여러 AI 지휘 도구](#3-독자적인-ai-에이전트-프레임워크--여러-ai-지휘-도구-22개) | 22 |
| 4 | [컴퓨터·브라우저를 대신 조작하는 AI](#4-컴퓨터브라우저를-대신-조작하는-ai-7개) | 7 |
| 5 | [AI에게 "기억력"을 달아주는 도구](#5-ai에게-기억력을-달아주는-도구-4개) | 4 |
| 6 | [메신저로 AI를 부르는 다리 (디스코드·텔레그램·카카오톡)](#6-메신저로-ai를-부르는-다리-디스코드텔레그램카카오톡-10개) | 10 |
| 7 | [작업 상태를 알려주는 알림·모니터링 도구](#7-작업-상태를-알려주는-알림모니터링-도구-3개) | 3 |
| 8 | [문서·자료를 찾고 정리해주는 도구](#8-문서자료를-찾고-정리해주는-도구-9개) | 9 |
| 9 | [발표자료·이미지·영상·디자인을 만드는 도구](#9-발표자료이미지영상디자인을-만드는-도구-35개) | 35 |
| 10 | [문서 파일 형식을 바꿔주는 변환 도구](#10-문서-파일-형식을-바꿔주는-변환-도구-5개) | 5 |
| 11 | [투자·금융 / 뉴스레터 자동화 도구](#11-투자금융--뉴스레터-자동화-도구-3개) | 3 |
| 12 | [보안 점검 도구](#12-보안-점검-도구-2개) | 2 |
| 13 | [터미널 화면을 여러 칸으로 나눠 쓰는 도구](#13-터미널-화면을-여러-칸으로-나눠-쓰는-도구-4개) | 4 |
| 14 | [그 외 각종 도구 (생활·교육·실험적 프로젝트)](#14-그-외-각종-도구-생활교육실험적-프로젝트-15개) | 15 |
...
터미널(검은 명령어 화면)에서 사람과 대화하듯 말을 걸면, 그 자체로 코드를 대신 짜주거나 컴퓨터 작업을 대신 해주는 "완결된 AI 프로그램"들입니다.
...
## 2. AI 코딩 비서를 더 강력하게 만들어주는 부속 도구 (63개)

위 1번 도구들(특히 claude-code, codex)에 "끼워 넣어" 기능을 더하거나, 여러 대를 편하게 관리해주는 부속품들입니다. 스마트폰 앱스토어에서 앱을 추가로 설치하는 것과 비슷합니다.
...
- **[ECC](https://github.com/SoDam-Fork/ECC)** *(원본: [affaan-m/ECC](https://github.com/affaan-m/ECC))* — Claude Code·Codex·OpenCode·Cursor 같은 AI 코딩 도구에 스킬, 기억, 보안 점검, 리서치 우선 개발 방식 등을 한꺼번에 더해주는 에이전트 하네스 성능 최적화 시스템.
- **[im-not-ai](https://github.com/SoDam-Fork/im-not-ai)** *(원본: [epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai))* — AI가 쓴 티가 나는 한국어를 찾아내고, 번역투·기계적인 반복 표현 등을 사람 글처럼 자연스럽게 다듬어주는 Claude 스킬.
- **[k-skill](https://github.com/SoDam-Fork/k-skill)** *(원본: [NomaDamas/k-skill](https://github.com/NomaDamas/k-skill))* — 한국 사용자를 위해 만든 에이전트 스킬 모음집. AI가 한국어·한국 환경에 더 맞게 작업하도록 돕는 부속 지식 세트입니다.
- **[solo-skills](https://github.com/SoDam-Fork/solo-skills)** *(원본: [bam-bam-2/solo-skills](https://github.com/bam-bam-2/solo-skills))* — 1인 사업자가 직원 없이 여러 업무를 자동화할 때 바로 쓸 수 있도록 만든 AI 에이전트 스킬 26개와 실행 스크립트 모음.
- **[claude-code-tips](https://github.com/SoDam-Fork/claude-code-tips)** *(원본: [ykdojo/claude-code-tips](https://github.com/ykdojo/claude-code-tips))* — Claude Code를 더 잘 쓰기 위한 45개 이상의 팁과 상태 표시줄 스크립트, 추가 스킬·플러그인 활용법을 모아둔 실전 가이드.
- **[claude-seo](https://github.com/SoDam-Fork/claude-seo)** *(원본: [AgriciDaniel/claude-seo](https://github.com/AgriciDaniel/claude-seo))* — 기술 SEO, E-E-A-T, Schema, GEO/AEO, 백링크, 로컬 SEO, 의미 기반 키워드 묶기 등 SEO 작업을 Claude Code에 맡길 수 있게 하는 종합 SEO 스킬 세트.
- **[stitch-skills](https://github.com/SoDam-Fork/stitch-skills)** *(원본: [google-labs-code/stitch-skills](https://github.com/google-labs-code/stitch-skills))* — Google의 Stitch MCP 서버와 함께 쓰도록 만든 Agent Skills 모음. Antigravity·Gemini CLI·Claude Code 등 여러 코딩 에이전트와 호환되는 공개 스킬 형식을 따릅니다.

---

## 3. 독자적인 AI 에이전트 프레임워크 / 여러 AI 지휘 도구 (22개)

AI 하나가 아니라 여러 AI(또는 여러 단계)를 팀처럼 조합해서 복잡한 일을 처리하게 만드는 "AI 조립 키트"들입니다. 보통 개발자가 이걸로 자기만의 AI 서비스를 새로 만듭니다.
...
- **[herdr](https://github.com/SoDam-Fork/herdr)** *(원본: [herdrdev/herdr](https://github.com/herdrdev/herdr))* — 여러 코딩 에이전트가 실제 작업을 실행하고 살아갈 수 있는 "런타임(실행 환경)"을 제공하는 에이전트 운영 기반 도구.
- **[orca](https://github.com/SoDam-Fork/orca)** *(원본: [stablyai/orca](https://github.com/stablyai/orca))* — 여러 코딩 에이전트를 동시에 병렬로 띄우고 관리하기 위한 ADE(Agent Development Environment). 각 AI 서비스의 기존 구독을 활용해 데스크톱·모바일·VPS에서 에이전트 함대를 운영하는 것을 목표로 합니다.
- **[openclaude](https://github.com/SoDam-Fork/openclaude)** *(원본: [Gitlawb/openclaude](https://github.com/Gitlawb/openclaude))* — 업로드된 GitHub 요약에는 "runs anywhere. uses anything"으로 소개된 AI 에이전트 프로젝트입니다. 여러 환경에서 실행하고 다양한 도구와 연결하는 방향을 내세우지만, 업로드 자료만으로 세부 기능은 더 구체적으로 확인되지 않습니다.

---

## 4. 컴퓨터·브라우저를 대신 조작하는 AI (7개)

마우스 클릭이나 화면 조작을, AI가 사람 대신 직접 해주는 도구들입니다.
...
## 5. AI에게 "기억력"을 달아주는 도구 (4개)

원래 AI는 대화가 끝나면 방금 나눈 이야기를 잊어버립니다. 이 도구들은 예전 대화·작업 내용을 저장해뒀다가 다음에 다시 꺼내 쓸 수 있게 해줍니다.
...
## 6. 메신저로 AI를 부르는 다리 (디스코드·텔레그램·카카오톡) (10개)

컴퓨터 앞에 앉아있지 않아도, 평소 쓰는 메신저 앱으로 AI 비서에게 말을 걸 수 있게 이어주는 "다리" 역할 도구들입니다.
...
- **[public-apis-4Kr](https://github.com/SoDam-Fork/public-apis-4Kr)** *(원본: [yybmion/public-apis-4Kr](https://github.com/yybmion/public-apis-4Kr))* — 한국에서 이용할 수 있는 공개 API들을 한곳에 모아 정리한 목록. 국내 서비스와 연결되는 API를 찾을 때 참고하는 자료 저장소입니다.

---

## 9. 발표자료·이미지·영상·디자인을 만드는 도구 (35개)

- **[ComfyUI_AI-Toolkit_Easy_Install](https://github.com/sodam-ai/ComfyUI_AI-Toolkit_Easy_Install)** *(원본: [Tavris1/AI-Toolkit-Easy-Install](https://github.com/Tavris1/AI-Toolkit-Easy-Install))* — AI 그림·영상 생성 프로그램 "ComfyUI"에서 쓰는 "AI-Toolkit"이라는 도구를, 윈도우에 클릭 한 번으로 설치해주는 설치 도구.
...
- **[bookforge](https://github.com/SoDam-Fork/bookforge)** *(원본: [gongnyang/bookforge](https://github.com/gongnyang/bookforge))* — 주제 한 줄만 주면 상업용 전자책 수준의 한국어 PDF를 만들도록 돕는 Claude Code·Codex 겸용 에이전트 스킬. 여러 스타일 팩과 배치 규칙, 품질 점검 단계를 포함합니다.
- **[diagram-design](https://github.com/SoDam-Fork/diagram-design)** *(원본: [cathrynlavery/diagram-design](https://github.com/cathrynlavery/diagram-design))* — Claude Code·Codex 등에서 편집용 다이어그램을 만들 때 참고할 수 있는 38가지 도식 디자인 유형 모음. HTML+SVG 방식으로 직접 그리는 구조입니다.
- **[reelforge](https://github.com/SoDam-Fork/reelforge)** *(원본: [gongnyang/reelforge](https://github.com/gongnyang/reelforge))* — 간단한 브리프 하나에서 내레이션·자막·장면 편집이 가능한 숏폼/롱폼 영상을 만드는 AI 영상 제작 파이프라인. 한국어 우선 설계와 HTML 기반 렌더링을 내세웁니다.
- **[store-screenshots](https://github.com/SoDam-Fork/store-screenshots)** *(원본: [LeeHueeng/store-screenshots](https://github.com/LeeHueeng/store-screenshots))* — 앱의 원본 스크린샷을 앱스토어·구글플레이에 올릴 수 있는 마케팅 이미지로 바꿔주는 Claude Code·Codex용 AI 에이전트 스킬. 다양한 스마트폰·태블릿 기기 프레임을 지원합니다.
- **[threeui](https://github.com/SoDam-Fork/threeui)** *(원본: [MengTo/threeui](https://github.com/MengTo/threeui))* — 웹에서 바로 확인할 수 있는 인터랙티브 UI 컴포넌트와 전체 소스 코드를 모아둔 오픈소스 ThreeUI 커뮤니티 카탈로그.
- **[watermarks-remover](https://github.com/SoDam-Fork/watermarks-remover)** *(원본: [guillaumemeyer/watermarks-remover](https://github.com/guillaumemeyer/watermarks-remover))* — 사용자가 소유한 콘텐츠에서 AI 워터마크를 제거하도록 만든 개인정보 보호 중심 앱. 저작권이나 이용약관을 지킬 수 있는 자기 소유 콘텐츠에 사용하는 것이 전제입니다.

---

## 10. 문서 파일 형식을 바꿔주는 변환 도구 (5개)
...
## 11. 투자·금융 / 뉴스레터 자동화 도구 (3개)

> ⚠️ 참고: 아래 도구는 투자·거래와 관련이 있습니다. 실제 투자 판단과 결과에 대한 책임은 사용자 본인에게 있으며, 이 설명은 참고용 정보 제공일 뿐 투자 조언이 아닙니다.
...
- **[ghostty](https://github.com/SoDam-Fork/ghostty)** *(원본: [ghostty-org/ghostty](https://github.com/ghostty-org/ghostty))* — 빠른 속도와 GPU 가속, 운영체제 기본 UI를 활용하는 크로스플랫폼 터미널 에뮬레이터. AI 전용 도구는 아니지만 개발·에이전트 작업용 터미널로 활용할 수 있습니다.

---

## 14. 그 외 각종 도구 (생활·교육·실험적 프로젝트) (15개)
...
- **[anatomy](https://github.com/sodam-ai/anatomy)** *(원본: [thebuggeddev/anatomy](https://github.com/thebuggeddev/anatomy))* — 사람 몸 구조를 3D로 돌려보며 배울 수 있는 인터랙티브 인체 해부학 학습 프로그램.
...
- **[open-command](https://github.com/SoDam-Fork/open-command)** *(원본: [tomdoyo/open-command](https://github.com/tomdoyo/open-command))* — 업로드된 GitHub 요약에는 "Open-source command reports!"라고 소개된 공개 프로젝트입니다. 제공된 자료만으로는 정확한 사용 목적과 기능 범위를 더 구체적으로 확정하기 어렵습니다.
- **[open-seo](https://github.com/SoDam-Fork/open-seo)** *(원본: [every-app/open-seo](https://github.com/every-app/open-seo))* — Semrush·Ahrefs 같은 상용 SEO 분석 서비스를 대체하는 것을 목표로 하는 오픈소스 SEO 도구.
...
- 이 문서는 2026-09-11 기준 갱신본입니다. 2026-09-07에 반영했던 신규 포크 3개에 더해, 이번에 업로드된 `langflow Public.md`와 대조해 기존 문서에 없던 20개를 추가하여 총 191개로 갱신했습니다. GitHub 저장소 목록은 계속 바뀔 수 있어서, 이 시점 이후 새로 포크하거나 지운 저장소는 반영되어 있지 않습니다.
- 기존 171개 설명은 각 저장소의 GitHub 공개 요약 문구·안내 문서(README)를 바탕으로 작성되어 있습니다. 이번 신규 20개는 업로드된 `langflow Public.md`에 적힌 저장소명, `Forked from` 원본, GitHub 공개 요약 문구를 기준으로 같은 형식에 맞춰 쉬운 말로 정리했습니다. 실제 저장소 안 코드 전체를 검수한 것은 아닙니다.
- 별표(⭐, star) 개수는 이번 정리에서 다루지 않았습니다. 필요하시면 "인기 많은 순으로 다시 정리해줘"라고 말씀해 주세요.
